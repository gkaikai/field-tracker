...
# AAPT2 路径：可被环境变量覆盖，默认取 build-tools 中最高版本
AAPT2="${AAPT2:-$(ls -d "$ANDROID_HOME/build-tools/"* 2>/dev/null | sort -V | tail -1)/aapt2}"
...
if [ -z "$TOKEN" ]; then
  ...
  ME_CODE=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" "$TUNNEL_URL/api/v1/auth/me" \
