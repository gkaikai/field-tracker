...
router.post('/rules',
  ...
  validate([
    ...
    body('startTime').optional().custom((v: any) => v === '' || /^([01]?\d|2[0-3]):[0-5]\d(:[0-5]\d)?$/.test(v)).withMessage('上班时间格式应为 HH:mm'),
    body('lateTime').optional().custom((v: any) => v === '' || /^([01]?\d|2[0-3]):[0-5]\d(:[0-5]\d)?$/.test(v)).withMessage('迟到时间格式应为 HH:mm'),
    body('endTime').optional().custom((v: any) => v === '' || /^([01]?\d|2[0-3]):[0-5]\d(:[0-5]\d)?$/.test(v)).withMessage('下班时间格式应为 HH:mm'),
    body('checkin_start').optional().custom((v: any) => v === '' || /^([01]?\d|2[0-3]):[0-5]\d(:[0-5]\d)?$/.test(v)).withMessage('上班时间格式应为 HH:mm'),
    body('checkin_end').optional().custom((v: any) => v === '' || /^([01]?\d|2[0-3]):[0-5]\d(:[0-5]\d)?$/.test(v)).withMessage('下班时间格式应为 HH:mm'),
    body('late_time').optional().custom((v: any) => v === '' || /^([01]?\d|2[0-3]):[0-5]\d(:[0-5]\d)?$/.test(v)).withMessage('迟到时间格式应为 HH:mm'),
  ]),
  async (req: Request, res: Response, next: NextFunction) => {
